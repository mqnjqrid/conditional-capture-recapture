...
for(alpha in seq(0.1, 0.5, by = 0.05)){
  for(omega in c(1)){print(c(alpha, omega))
    for(s in 1:simuldraw){
      ...
      result1 <- rbind(result1, cbind(est_val$vvec, est_val$plugin, est_val$drl, est_val$plugin.or, est_val$drl.or, est_val$drlk, est_val$drlk.or, est_val$gammainv.or, alpha, omega)[est_val$set1 == 2,])
      #result2 <- rbind(result2, cbind(est_val$vvec, est_val$plugin.or, est_val$drl.or, est_val$gammainv.or, alpha, omega))
      
    }
  }
}
colnames(result1)[(length(v)+1):ncol(result1)] = c("plugin", "drl", "plugin.or", "drl.or", "drlk", "drlk.or", "gammainv.or", "alpha", "omega")
...
g1 <- ggplot(resultmse[resultmse$variable == "mse",], aes(x = alpha, y = value, color = method)) +
  ...
  #facet_wrap(~variable) +
  scale_color_manual(values = c("plugin" = "red", "drl" = "goldenrod", "drl.or" = "grey", "drlk" = "green", "drlk.or" = "blue"))
