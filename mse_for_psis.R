...
for(it in c(3:11)){
  ...
  for(omega in c(1)){print(c(it, omega))
    for(s in 1:simuldraw){
      ...
      est_val = estim_pseudoor(List_matrix, n = n0, K = K, v = v, alpha = alpha, omega = omega, plot = TRUE)
      
      result1 <- rbind(result1, cbind(est_val$vvec, est_val$plugin, est_val$drl, est_val$plugin.or, est_val$drl.or, est_val$gammainv.or, alpha, omega, n0, round(theta0, 2))[est_val$set1 == 2,])
      #result2 <- rbind(result2, cbind(est_val$vvec, est_val$plugin.or, est_val$drl.or, est_val$gammainv.or, alpha, omega))
      
    }
  }
}
colnames(result1)[(length(v)+1):ncol(result1)] = c("plugin", "drl", "plugin.or", "drl.or", "gammainv.or", "alpha", "omega", "n0", "theta0")
